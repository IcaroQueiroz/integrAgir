...
class AppCielo():
    def cielo(self):
        try: 
            ...
            self.df['Data da venda'] = pd.to_datetime(self.df['Data da venda'])  # Converta a coluna para o tipo de dados datetime, se ainda não estiver
            self.df['Data da venda'] = self.df['Data da venda'].dt.date  # Extraia apenas a parte da data, excluindo a hora
            self.df['DataNat'] = self.df['Data da venda'].astype(str) + self.df['Bandeira'].astype(str) + self.df['Natureza'].astype(str) # Concatene a data (sem hora) com a coluna 'Natureza'

            # Agrupando e somando os valores
            self.df_agrupado = self.df.groupby(['Data da venda', 'Bandeira', 'Natureza', 'DataNat']).agg({'Valor da venda': 'sum', 'Valor líquido da venda': 'sum', 'DataNat': 'count'}).rename(columns={'DataNat': 'Quantidade'}).reset_index()
            self.df_agrupado['Taxa'] = self.df_agrupado.apply(lambda row: row['Valor da venda'] - row['Valor líquido da venda'], axis=1)
            ...
            # Calcular os totais
            total_credito = dados_credito['Valor da venda'].sum()
            total_debito = dados_debito['Valor da venda'].sum()
            total_voucher = dados_voucher['Valor da venda'].sum()
            ...
            for i, controlador in enumerate (self.df_agrupado['DataNat']):
                data_str = self.df_agrupado.loc[i,'Data da venda']
                ...
                lanSimp = "000001,"+ str(data_txt) + ","+str(conta_ativo)+",18," + str(self.df_agrupado.loc[i,'Valor da venda']) + ",00000000," + "Vlr. Ref. Cartão "+ str(self.df_agrupado.loc[i,'Bandeira'])+" "+ str(self.df_agrupado.loc[i,'Natureza']) + " - Nº de Vendas:"+ str(self.df_agrupado.loc[i,'Quantidade']) +",,"+ cpfoucnpj + "," + "\n"
                lanSimpT = "000001,"+ str(data_txt) + ","+str(conta_despesa)+","+str(conta_ativo)+"," + str(self.df_agrupado.loc[i,'Valor da venda']) + ",00000000," + "Vlr. Ref. Cartão "+ str(self.df_agrupado.loc[i,'Bandeira'])+" "+ str(self.df_agrupado.loc[i,'Natureza']) + " - Nº de Vendas:"+ str(self.df_agrupado.loc[i,'Quantidade']) +",,"+ cpfoucnpj + "," + "\n"
